...
const VENUE_TYPES: VenueType[] = [
  {
    ...
    name: 'All Venues',
    icon: Building,
    description: 'All venue types',
  },
  {
    id: 'jazz',
    name: 'Jazz Clubs',
    icon: Music,
    description: 'Jazz and blues venues',
  },
  {
    id: 'rock',
    name: 'Rock Venues',
    icon: Building,
    description: 'Rock and indie venues',
  },
  {
    id: 'coffee',
    name: 'Coffee Shops',
...
export default function VenueDiscovery() {
  ...
  const fetchVenues = useCallback(async () => {
    ...
    try {
      ...
      if (selectedVenueType !== 'all') {
        params.append('venue_type', selectedVenueType);
      }
      if (capacityRange[0] > 0) {
        params.append('min_capacity', capacityRange[0].toString());
      }
      if (capacityRange[1] < 2000) {
        params.append('max_capacity', capacityRange[1].toString());
      }
  ...
  const getVenueTypeIcon = (venue: Venue) => {
    ...
    if (genres.some(g => ['jazz', 'blues'].includes(g))) {
      return Music;
    }
    if (genres.some(g => ['rock', 'indie', 'alternative'].includes(g))) {
      return Building;
    }
    if (venue.capacity && venue.capacity < 100) {
      return Coffee;
    }
    return Star;
  };
        ...
        <div className='max-w-7xl mx-auto py-8 px-4'>
          ...
          <div className='mb-8'>
            ...
            <p className='text-gray-600'>
              Find the perfect venues for your music
            </p>
          </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className='flex items-start justify-between mb-4'>
                        ...
                        >
                          <Heart
                            className={`w-5 h-5 ${favorites.includes(venue.id) ? 'fill-red-500 text-red-500' : ''}`}
                          />
                        </button>
                      </div>
                        ...
                        <button
                          onClick={e => {
                            ...
                            navigator.clipboard.writeText(
                              `${venue.name} - ${venue.city}${venue.website ? ` - ${venue.website}` : ''}`
                            );
                          }}
