...
export default function OutreachTracker() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (profileData) {
        ...
        // Fetch analytics
        const statsRes = await fetch(`/api/outreach/stats?artist_id=${profileData.id}`);
        const analyticsData = await statsRes.json();
        setAnalytics(analyticsData);

        // Fetch campaigns
        const campaignsRes = await fetch(`/api/outreach/campaigns?user_id=${profileData.id}`);
        const campaignsData = await campaignsRes.json();
        setCampaigns(campaignsData.campaigns || []);

        // Fetch emails
        const emailsRes = await fetch(`/api/outreach/emails?artist_id=${profileData.id}`);
  ...
  const handleSendEmail = async () => {
    if (!profile || !newEmail.venue_name || !newEmail.subject || !newEmail.body) return;
  ...
  const filteredEmails = emails.filter(email => {
    if (selectedCampaign !== 'all' && email.campaign_id !== selectedCampaign) return false;
    if (emailStatus !== 'all' && email.status !== emailStatus) return false;
    return true;
  });
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-600 mt-2">Track your booking pitches and responses</p>
          </div>
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm border">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-900">{analytics.stats.total_emails}</p>
                </div>
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm border">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-900">{analytics.stats.response_rate}%</p>
                </div>
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm border">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-900">{analytics.campaigns.active}</p>
                </div>
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm border">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-900">{analytics.followups.pending}</p>
                </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border overflow-hidden">
                ...
                {filteredEmails.map((email) => (
                  <tr key={email.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div>
                        <div className="text-sm font-medium text-gray-900">{email.venue_name}</div>
                        <div className="text-sm text-gray-500">{email.venue_city}</div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      {email.response_type ? (
                        <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                          email.response_type === 'positive' ? 'bg-green-100 text-green-800' :
                          email.response_type === 'negative' ? 'bg-red-100 text-red-800' :
          ...
          {filteredEmails.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-gray-500">No emails found. Start by sending your first pitch!</p>
            </div>
          )}
        </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setNewEmail({...newEmail, venue_name: e.target.value})}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setNewEmail({...newEmail, venue_email: e.target.value})}
                      className="w-full border border-gray-300 rounded px-3 py-2 focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setNewEmail({...newEmail, venue_city: e.target.value})}
                      className="w-full border border-gray-300 rounded px-3 py-2 focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setNewEmail({...newEmail, campaign_id: e.target.value})}
                    className="w-full border border-gray-300 rounded px-3 py-2 focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                  >
